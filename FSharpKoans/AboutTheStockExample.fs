...
module ``about the stock example`` =
    ...
    let YouGotTheAnswerCorrect() =
        let difference(x:string, y:string) =
            abs(Double.Parse(x, CultureInfo.InvariantCulture) - Double.Parse(y, CultureInfo.InvariantCulture))

        let result =
            stockData.Tail
            |> List.map (fun (x:string) -> x.Split(','))
            |> List.map (fun (x:string[]) -> [| x.[0]; x.[1]; x.[4] |])
            |> List.map (fun (x:string[]) -> [| x.[0]; string(difference(x.[1], x.[2])) |])
            |> List.maxBy (fun (x:string[]) -> Double.Parse(x.[1], CultureInfo.InvariantCulture))
            |> Array.item 0
